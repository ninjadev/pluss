...
vec2 sceneSDF(vec3 samplePoint) {    
    ...
    vec2 box = vec2(boxSDF(samplePoint, vec3(2.)), 1.);
    vec3 repeatPoint = samplePoint + vec3(1., -1., 0.);
    repeatPoint.x = mod(repeatPoint.x + 1., 2.) - 1.;
    repeatPoint.y = mod(repeatPoint.y + 1., 2.) - 1.;
    vec2 shell = vec2(boxSDF(repeatPoint, vec3(0.05, 0.05, 2.)), 4.);

    vec3 repeatPoint2 = samplePoint + vec3(0., -1., 1.);
    repeatPoint2.z = mod(repeatPoint2.z + 1., 2.) - 1.;
    repeatPoint2.y = mod(repeatPoint2.y + 1., 2.) - 1.;
    vec2 shell2 = vec2(boxSDF(repeatPoint2, vec3(2., 0.05, 0.05)), 4.);

    vec3 repeatPoint3 = samplePoint + vec3(1., 0., 1.);
    repeatPoint3.x = mod(repeatPoint3.x + 1., 2.) - 1.;
    repeatPoint3.z = mod(repeatPoint3.z + 1., 2.) - 1.;
    vec2 shell3 = vec2(boxSDF(repeatPoint3, vec3(0.05, 2., 0.05)), 4.);
    shell = unionSDF(unionSDF(shell, shell2), shell3);
    box = intersectSDF(unionSDF(box, shell), vec2(boxSDF(samplePoint, vec3(3.)), 0.));
    vec2 inside = vec2(octahedral(samplePoint, 31., 1.25), 1.);
    vec2 difference = differenceSDF(box, inside);
    float wobble =  0.01 * sin(samplePoint.x * 11. + frame / 10.) + 0.01 * sin(samplePoint.z * 8. + frame / 20.);

    vec2 ground = vec2(boxSDF(samplePoint + vec3(0., sqrt(2.), 0.), vec3(100., 1., 100)), 2.);

    vec3 r = rotateX(frame / 20.) * samplePoint;
    r = rotateY(frame / 30.) * r;
    vec2 middle = unionSDF(
        vec2(boxSDF(r, (1. + 1.5 * blast) * vec3(0.1, 0.5, 0.1)), 3.),
        unionSDF(
            vec2(boxSDF(r, (1. + 1.5 * blast) * vec3(0.1, 0.1, 0.5)), 3.),
            vec2(boxSDF(r, (1. + 1.5 * blast) * vec3(0.5, 0.1, 0.1)), 3.)));


    vec2 result;
    if(frame > 2300.) {
        result = unionSDF(middle, unionSDF(difference, ground));
    } else {
        result = unionSDF(difference, ground);
    }
    return result;
}
...
void main() {
    ...
    if(frame > 2246.75) {
        eye = vec3(5.0,-.5, 5.0);
    } else if(frame > 2095.75) {
        eye = vec3(8.0, 5.0, 7.0);
    } else {
        eye = vec3(2.0, 3.0, 7.0);
    }
    ...
    color += (1. + blast) * pink * (1. - ambientOcclusion(p, N, 0.001));
