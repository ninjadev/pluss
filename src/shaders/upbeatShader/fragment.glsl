...
void main() {
    ...
    if (scene < 2.) {
        ...
        for (float i = 0.; i < 1.; i += 0.025) {
            ...
            offset *= easeOutSin(time) * 6. * (0.5 + hash(i) * 0.5);

            particles = min(particles, thingy(p, offset, hash(i + 2424.) * 0.1, hash(i + 1337.), hash(i + 666.)));
        }
        ...
        col = (
            vec3(1.0, p.y, 0.25) * bg +
            vec3(1., 1., 0.) * (1. - spiral) * particles * particle_outline +
